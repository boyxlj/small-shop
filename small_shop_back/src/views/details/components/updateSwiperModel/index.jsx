...
import { selectShopAnyOne ,deleteShopSwiperAnyOne,editorSwiperImg} from "../../../../api/request"
...
export default function UpdateSwiperModel(props) {
  const dispatch = useDispatch()
  const isShowEditorSwiper = useSelector(state => state.detailOrEditor.isShowEditorSwiper)
  const [swiperList, setSwiperList] = useState([])
  const [loading, setLoading] = useState(false)
  const { detailId, reLoad } = props
  ...
  const removeSwiper = async(id)=>{
    ...
    const {data:res} = await deleteShopSwiperAnyOne(id)
          ...
          <>
            <Radio.Group className={style.radio} style={{margin:'5px 0 20px 0'}}   onChange={onChange} value={value}>
             ...
             {value==1 &&(
                 ...
                 <Button  className={style.swiperInputAdd} loading={loading} onClick={addSwiper} type='primary'>添加</Button>
               </div>
             )}   
          {swiperList.length<6 && value==2 && (
             <Upload
             maxCount={1}
             action={`${uploadImgUrl}?detailId=${detailId}`}
